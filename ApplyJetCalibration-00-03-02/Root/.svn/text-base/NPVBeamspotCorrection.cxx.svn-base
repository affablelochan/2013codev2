 ...
 * Adapted for ApplyJetCalibration by Joe Taenzer <joseph.taenzer@cern.ch>
 *
...
NPVBeamspotCorrectionTool::NPVBeamspotCorrectionTool()
  : g_nvtx_nreco_bs66mm(0), g_nvtx_nreco_bs47mm(0)
{
...
double NPVBeamspotCorrectionTool::GetNVertexBsCorrection(double nRecoVtx) {
  static TGraph *invGraph = NULL;
  if (invGraph==0) {
    invGraph = new TGraph();
    for (int i=0;i<g_nvtx_nreco_bs66mm->GetN();++i) 
      invGraph->SetPoint(i,g_nvtx_nreco_bs66mm->GetY()[i],g_nvtx_nreco_bs66mm->GetX()[i]);
  }
  static double NPVmin = g_nvtx_nreco_bs66mm->GetY()[0];
  static double NPVmax = g_nvtx_nreco_bs66mm->GetY()[g_nvtx_nreco_bs66mm->GetN()-1];
...
double NPVBeamspotCorrectionTool::GetNVertexBsCorrection(double nRecoVtx) {
  ...
  if (nRecoVtx < g_nvtx_nreco_bs66mm->GetY()[0]) {
    std::cout << "WARNING - NPVBeamspotCorrectionTool: Requested nVertex outside the expected range: " << nRecoVtx << std::endl;
...
*/
TGraph* NPVBeamspotCorrectionTool::NVtx_NReconstructible_bs47mm() {
...
TGraph* NPVBeamspotCorrectionTool::NVtx_NReconstructible_bs66mm() {
   TGraph *graph = new TGraph(41);
...
void NPVBeamspotCorrectionTool::initNPVBeamspotCorrection() {

  printf("\n\n");
  printf("===================================\n\n");
  printf("  Initializing the NPV Beamspot correction tool\n");
