// $Id: TrigConfigSvcD3PD.cxx 597982 2014-05-20 14:15:58Z tamartin $
...
namespace D3PD {
    ...
    */
    const std::string& TrigConfigSvcD3PD::GetAlgNameFromSeqIDAndAlgPos( int _sequenceID, int _algorithmPosition ) const {
      ...
      std::pair<int,int> _toFind = std::make_pair<int,int>( _sequenceID, _algorithmPosition );
   ...
   void TrigConfigSvcD3PD::GenerateSignatureSequenceMap(std::pair<int, std::pair<int, int> > _keys) {
     ...
     Info("TrigConfigSvcD3PD::GenerateSignatureSequenceMap", "Forming trigger configuration helper maps");
      ...
      for(unsigned int s = 0; s < GetSeqN(); ++s) {
         ...
         _seqMap->second.insert( std::make_pair<unsigned int, std::string>( GetSeqIndex(s), GetSeqName(s)) );
         // For all sequence algs
         for(unsigned int a = 0; a < GetAlgN(s); ++a) {
            std::pair<int,int> _algID = std::make_pair<int,int>( (int)GetSeqIndex(s) , (int)GetAlgPosition(s,a) );
            // Map algorithm name, name ID, class name and class name ID to location ID (combination of sequence ID and algorithm position).
            _algNameMap  ->second.insert( std::make_pair< std::pair< int, int > , std::string > ( _algID , GetAlgName(s,a) ) );
            _algNameIDMap->second.insert( std::make_pair< std::pair< int, int > , unsigned int >( _algID , GetAlgNameID(s,a) ) );
            _algTypeMap  ->second.insert( std::make_pair< std::pair< int, int > , std::string > ( _algID , GetAlgTypeName(s,a) ) );
            _algTypeIDMap->second.insert( std::make_pair< std::pair< int, int > , unsigned int >( _algID , GetAlgTypeID(s,a) ) );
