...
parser = OptionParser()
parser.add_option("-i", "--input",          help="file to study",                           action="store", type="string",  dest="input",   default=False)
...
#assert options.eos_dir or options.input, "no eos folder specified (use -e) and no input file (use -i)"
...
if options.files:
    ...
    for file in file_list:
        #print file
        try:
            #run = file.split(".")[1].strip("\n")
            run = file.split("dpd-")[1].split(".root")[0]
        except IndexError:
            run = file.strip("\n")
...
"""
if options.input:       file_list = [options.input]
elif options.glados:    file_list = make_file_list(options.eos_dir,grep=options.token,verbose=options.verbose)
...
for i,run in enumerate(runs):
    ...
    for file in file_list:
        if run in file: 
            file = file.split("\n")[0]
            #print file 
            this_file_list.append(file)
            used_files.append(file)
        
    #file_list = make_eos_file_list(options.eos_dir,grep=run,verbose=options.verbose)
    print "[%3i/%3i] - got %i files from work folder for run: %s"%(i+1,len(runs),len(this_file_list),run)
...
for i,run in enumerate(runs):
    ...
    print "%-10s %15i %15i %15i %15i %15i %15i %15i %15i %15i"%\
     (run,run_information[run][1],run_information[run][2],run_information[run][3],run_information[run][4],run_information[run][5],run_information[run][6],run_information[run][7],run_information[run][14],run_information[run][34])
...
print "\n=== %i of %i files used"%(len(used_files),len(file_list))
print "unused files:"
for file in file_list:
    if file.split("\n")[0] not in used_files: 
        command = "xrd eosatlas rm %s"%file
        print command
        #print "mv %s ../unused/ "%file.split("/").pop()
