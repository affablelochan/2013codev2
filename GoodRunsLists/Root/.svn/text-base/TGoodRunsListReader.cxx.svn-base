...
ClassImp(Root::TGoodRunsListReader)

using namespace std;


Root::TGoodRunsListReader::TGoodRunsListReader( Bool_t checkGRLInfo )
 : TObject()
...
Root::TGoodRunsListReader::TGoodRunsListReader( const TString& dataCardName, Bool_t checkGRLInfo )
 : TObject()
 , m_logger( "TGoodRunsListReader" )
{
...
void
Root::TGoodRunsListReader::ReadAttribs( TXMLNode* node )
{
...
void
Root::TGoodRunsListReader::ReadLumiBlockCollection( TXMLNode* dataNode )
{
...
Root::TGoodRun
Root::TGoodRunsListReader::GetLumiBlockCollection( TXMLNode* dataNode )
{
...
void
Root::TGoodRunsListReader::ReadNamedLumiRange( TXMLNode* dataNode )
{
  // sanity check
  if (!dataNode->HasChildren()) {
     m_logger << kWARNING << "<Data> ... </Data> Part does not contain any parameters" << GEndl;
  ...
  // retrieve childrens of node
  TXMLNode* node = dataNode->GetChildren();
  Root::TGoodRunsList grl(m_dataCardName.Data());
  ...
  for (; node != 0; node = node->GetNextNode()) {
    ...
    /// set run and lb range
      ...
      Root::TGoodRun goodrun = this->GetLumiBlockCollection(node);
...
Root::TGoodRunsListReader::GetMergedGoodRunsList( const Root::BoolOperation& operation ) const
{
...
const Root::TGoodRunsList
Root::TGoodRunsListReader::GetGoodRunsList( unsigned int idx ) const
{
...
Root::TGoodRunsListReader::GetMergedGRLCollection( const Root::BoolOperation& operation ) const
{
